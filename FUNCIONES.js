...
function obtener_num_intento_jugador(id) { //NO anda correctamente
    ...
    if ( num.length != 4){ //entra pero no ejecuta el contenido
       alert ("EL NUMERO DEBE CONTENER 4 CIFRAS - Ingreselo de nuevo");
        ...
        if ( aux1==aux2 || aux1==aux3 || aux1==aux4 || aux2==aux3 || aux2==aux4 || aux3==aux4 ){ //entra pero no ejecuta el contenido
            ...
            intento_jugador.num1 = Number(aux1);//no funciona
            intento_jugador.num2 = Number(aux2);//no funciona
            intento_jugador.num3 = Number(aux3);//no funciona
            intento_jugador.num4 = Number(aux4);//no funciona
            control++;
    ...
    if (control==1) { //entra pero no ejecuta el contenido
        ...
        ctx.fillStyle = "brown";
        if ( intentos>12 ){
            document.getElementById('comentarios').style.height = 500 + (30*(intentos-11));
        }
